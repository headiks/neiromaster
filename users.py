...
Хранилище: data/users.json. Открытых паролей на диске нет — только соль и scrypt-хэш.
"""
...
def list_users() -> list:
    with _lock:
        users = _load()
    return sorted((public_view(u) for u in users.values()),
...
def set_username(user_id: str, username: str) -> dict:
    ...
    with _lock:
        ...
        if any(u.get("username") == username and u["id"] != user_id for u in users.values()):
            raise ValueError(f"Логин «{username}» уже занят")
...
def transfer_ownership(current_owner_id: str, new_owner_id: str) -> dict:
    ...
    with _lock:
        users = _load()
        current = users.get(current_owner_id)
...
def migrate_legacy_employees() -> int:
    ...
    with _lock:
        users = _load()
        existing_ids = set(users)
        for employee in legacy.values():
            if employee.get("id") in existing_ids:
                continue
